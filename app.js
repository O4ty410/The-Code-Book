...
const FLOOR_MESSAGES = [
  { icon: '\u1F393', message: 'You understand how the internet works, how computers think, and the logic behind all code. That\'s the foundation everything else is built on.' },
  { icon: '\u1F3A8', message: 'You can build real webpages that look good. HTML and CSS are yours now. The visual web is no longer a mystery.' },
  { icon: '\u26A1', message: 'JavaScript. You made things move, respond, and think. This is where most people stop. You didn\'t.' },
  { icon: '\u1F528', message: 'You built alone. No hand-holding. A real brief, a real product. That\'s the developer mindset.' },
  { icon: '\u1F310', message: 'Frontend, backend, databases, APIs, deployment. You are a full stack developer. Let that land.' },
  { icon: '\u1F3AF', message: 'You found your lane and went deep. This is where developers become specialists.' },
  { icon: '\u1F3C6', message: 'Floor 7. Professional grade. You started from nothing and built your way here. That distance is yours forever.' }
];
...
function showStreakRecovery() {
  ...
  banner.innerHTML = `
    <div style="font-size:24px;margin-bottom:8px;">\u1F525</div>
...
function showProgressNudge(fi, si) {
  ...
  nudge.innerHTML = `
    <div style="font-size:20px;margin-bottom:8px;">\u1F989</div>
...
function showGuestSavePrompt() {
  ...
  prompt.innerHTML = `
    <div style="background:var(--surface);border:1px solid var(--accent);border-radius:16px;padding:32px;max-width:340px;text-align:center;">
      <div style="font-size:40px;margin-bottom:16px;">\u1F989</div>
...
const SAGE_MOODS = {
  encourage: { icon: '\u1F989', color: 'var(--accent)' },
  tip:       { icon: '\u1F989', color: 'var(--accent2)' },
  warn:      { icon: '\u1F989', color: 'var(--floor3)' },
  celebrate: { icon: '\u1F989', color: 'var(--success)' }
};
...
function renderBuildPanel() {
  ...
  var projects = [
    {
      icon: '\u1F310', floor: 1, title: 'My First Webpage', desc: 'Build a personal "About Me" page using only what you learned in Floor 1.',
    ...
    {
      icon: '\u1F3A8', floor: 2, title: 'Styled Portfolio Card', desc: 'Take your About Me page and make it look like something real.',
    ...
    {
      icon: '\u1F4CB', floor: 4, title: 'To-Do List App', desc: 'A working to-do list that remembers your tasks after refresh.',
    ...
    {
      icon: '\u1F324\uFE0F', floor: 5, title: 'Weather Dashboard', desc: 'Fetch live weather data from an API and display it in a custom UI.',
    ...
    {
      icon: '\u1F680', floor: 6, title: 'Full Portfolio Site', desc: 'A multi-page portfolio site that represents you professionally.',
    ...
    {
      icon: '\u1F3D7\uFE0F', floor: 7, title: 'Capstone Project', desc: 'Your final project. No constraints. Build something you\'re proud of.',
...
function renderChallengePanel() {
  ...
  var challenges = [
    ...
    { icon: '\u1F9E0', type: 'RECALL', title: 'Spaced Repetition Quiz', desc: 'Questions from sections you completed. Reinforce what you know.', xp: '+15 XP each', action: 'startRecallQuiz()', done: false },
    ...
    { icon: '\u1F525', type: 'STREAK', title: 'Streak Challenge', desc: 'Answer 5 questions in a row without getting one wrong.', xp: '+75 XP', action: 'startStreakChallenge()', done: false, locked: state.xp < 200 },
    { icon: '\u1F3C6', type: 'FLOOR', title: 'Floor Boss', desc: 'A comprehensive quiz on everything in the floor you just completed.', xp: '+100 XP', action: 'startFloorBoss()', done: false, locked: !isFloorComplete(state.currentFloor - 1) },
  ];
...
function _endSpeedRound() {
  ...
  document.getElementById('challenge-question').textContent = score >= 8 ? '\u1F3C6 Excellent!' : score >= 5 ? '\u1F44D Good effort!' : '\u1F4AA Keep practising!';
...
function _endStreakChallenge() {
  ...
  document.getElementById('challenge-question').textContent = !_streakFailed ? '\u1F525 Flawless!' : _streakCorrect >= 3 ? '\u1F44D Good run!' : '\u1F4AA Try again!';
  document.getElementById('challenge-options').innerHTML = '<button class="auth-btn" onclick="closeDailyChallenge()" style="margin-top:8px;">Done</button>';
  document.getElementById('challenge-result').style.display = 'none';
}
...
function _endFloorBoss() {
  ...
  document.getElementById('challenge-question').textContent = _bossScore === _bossTotal ? '\u1F3C6 Perfect \u2014 Floor ' + state.currentFloor + ' mastered!' : _bossScore >= 3 ? '\u1F44D Solid knowledge!' : '\u1F4D6 Revisit this floor.';
  document.getElementById('challenge-options').innerHTML = '<button class="auth-btn" onclick="closeDailyChallenge()" style="margin-top:8px;">Done</button>';
  document.getElementById('challenge-result').style.display = 'none';
}
